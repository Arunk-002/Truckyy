...
function ReviewsTab({ truckId }) {
  ...
  const onSubmitReview = async (e) => {
    ...
    try {
      ...
      const response = await axiosInstance.post(`/users/add-review/${user._id}`, reviewData);

      if (response.status === 201) {
        setSubmitMessage({ type: 'success', message: 'Your review has been submitted!' });
        resetForm();
    ...
    <div className="space-y-8">
      ...
      <div className="space-y-6">
        ...
        {reviews.length === 0 ? (
          ...
          reviews.map((review) => (
            <div key={review._id || review.id} className="bg-white border border-gray-100 rounded-lg p-4">
              <div className="flex items-start space-x-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="mt-2 text-gray-700">{review.comment || review.content}</p>
                </div>
              </div>
            </div>
          ))
